...
"""
crackme stego tool
- AES-256-GCM (PBKDF2-SHA256, 600k iter) шифрует флаг паролем
- шифртекст зашивается в LSB пикселей PNG, порядок пикселей определяется
  PRNG, посеянным от пароля -> без пароля даже не знаешь ГДЕ искать биты,
  не то что как их собрать в правильном порядке.
- Использовать только с PNG (lossless), JPEG убьёт LSB при пересжатии.
...
def encrypt(plaintext: bytes, password: str):
    ...
    # payload: MAGIC | salt(16) | nonce(12) | ct(includes 16-byte GCM tag)
...
def embed(in_path, out_path, password, secret_text):
    ...
    capacity = flat.shape[0] * 3
    capacity = flat.shape[0]
    if len(bits) > capacity:
        raise ValueError(f"Картинка слишком маленькая: нужно {len(bits)} пикселей, есть {capacity}")

    # порядок пикселей и канал зависят от пароля + соли фиксированной (нужна для order, отдельно от AES соли)
    order_salt = hashlib.sha256(password.encode()).digest()[:8]
    order = pixel_order(w, h, password, order_salt)

    chosen = order[:len(bits)]
    # пишем в синий канал (индекс 2) — наименее заметно для глаза
    ...
    print(f"[+] Пароль для встраивания/извлечения: {password}")

def extract(in_path, password):
    img = Image.open(in_path).convert("RGB")
    arr = np.array(img)
    h, w, _ = arr.shape
    flat = arr.reshape(-1, 3)
    ...
    # сначала вытаскиваем 32 бита длины
...
def main():
    ...
    if args.cmd == "embed":
        ...
        try:
            data = extract(args.input, args.password)
